...
def preprocess_datasets(config, dataset):
...
def load_model_for_text_generation(config):
...
def get_tokenizer_for_text_generation(config, dataset):
    ...
    def tokenize_fn(examples):
        ...
        # We keep `labels` identical to `input_ids`
        return {"input_ids": input_ids, "attention_mask": attention_mask, "label": input_ids.copy()}

    # Apply `tokenize_fn` to train and validation splits, remove the original "text" column
...
def load_new_model_for_sequence_classification_from_local_path(config):
    ...
    if config["fttype"] == "ttlora" or config["fttype"] == "lora":
